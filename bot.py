#!/usr/bin/env python3
"""
Driver Bot — Full: Missions, Reports, Maintenance, Admin Inline Finance Form
- Missions header auto-repair
- Robust roundtrip merge (multi-city supported)
- Annual/Monthly/Weekly stats & reports
- Unfinished mission detection and reminders
- Driver leave records (Driver_Leave)
- Vehicle maintenance records (Vehicle_Maintenance)
- Admin inline finance form for Odometer / Fuel / Parking
- Sheet auto-repair and cleanup
"""
...
# ===== Roundtrip summary functions (already above) =====
# (count_roundtrips_per_driver_month / write_roundtrip_summary_tab / write_roundtrip_summary_csv defined earlier)
...
# show initial admin finance menu (command)
...
async def admin_finance_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) >= 5 and parts[1] == "amount":
        ...
        if amount != "custom":
            ...
            try:
                if typ == "odo":
                    res = add_vehicle_maintenance(plate, today_date_str(), amount, "Odometer", "", "")
                elif typ == "fuel":
                    res = add_vehicle_maintenance(plate, today_date_str(), amount, "Fuel", "", "")
                elif typ == "parking":
                    res = add_vehicle_maintenance(plate, today_date_str(), amount, "Parking", "", "")
                else:
                    res = {"ok": False}
                if res.get("ok"):
            ...
            # Custom amount: ask user to reply in chat (ForceReply). Save pending in user_data.
...
async def process_finance_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if initiator and user and user.id != initiator:
        return  # ignore replies from others
    typ = pending.get("typ")
    ...
    try:
        if typ == "odo":
            res = add_vehicle_maintenance(plate, today_date_str(), amount, "Odometer", "", notes)
        elif typ == "fuel":
            res = add_vehicle_maintenance(plate, today_date_str(), amount, "Fuel", "", notes)
        elif typ == "parking":
            res = add_vehicle_maintenance(plate, today_date_str(), amount, "Parking", "", notes)
        else:
            res = {"ok": False}
        if res.get("ok"):
...
# ===== Other handlers: plate_callback, location_or_skip, lang, mission_report etc. =====
async def plate_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # admin_finance button used earlier; now we handle via group_admin_finance_command or admin_finance_callback
...
def register_ui_handlers(application):
    ...
    # Admin inline finance callback handler
    application.add_handler(CallbackQueryHandler(admin_finance_callback, pattern=r'^fin\|'))
    # Message handler for processing custom finance replies (should be early)
    ...
    # admin finance command to start flow
    application.add_handler(CommandHandler("admin_finance", group_admin_finance_command))

    # direct admin commands (fallback; they will be checked for admin status inside)
        ...
        async def _set_cmds():
            try:
                await application.bot.set_my_commands([
                    ...
                    BotCommand("admin_finance", "Show admin finance buttons"),
                ])
            except Exception:
