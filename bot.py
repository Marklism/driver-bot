...
def _load_creds_from_base64(encoded: str) -> dict:
    try:
        ...
        logger.exception("Failed to decode GOOGLE_CREDS_B64")
        raise

def get_gspread_client():
    creds_json = None
    if GOOGLE_CREDS_B64:
        creds_json = _load_creds_from_base64(GOOGLE_CREDS_B64)
...
async def debug_bot_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    google_creds = bool(os.getenv("GOOGLE_CREDS_B64") or os.getenv("GOOGLE_CREDS_B64") or os.getenv("GOOGLE_CREDS_PATH"))
...
async def _send_startup_debug(application):
    ...
    try:
        ...
        lines.append(f"Google creds present: {'Yes' if (os.getenv('GOOGLE_CREDS_B64') or os.getenv('GOOGLE_CREDS_B64') or os.getenv('GOOGLE_CREDS_PATH')) else 'No'}")
        # list commands
        try:
            if bot_token:
                b = Bot(bot_token)
...
# ---- APPENDED HELPERS: safe gspread wrappers and mission pairing (no triple quotes) ----
...
def _make_gspread_client_from_b64(b64_env_name='GOOGLE_CREDS_B64'):
    creds_b64 = os.environ.get(b64_env_name)
    if not creds_b64 or gspread is None or Credentials is None:
        logging.debug('No GOOGLE_CREDS_B64 or gspread not available.')
        return None
    try:
        ...
        logging.exception('gspread RefreshError: invalid scope or audience. Falling back to None.')
        return None
    except Exception:
        logging.exception('gspread client creation failed.')
        return None

def save_mission_cycles_to_sheet(mission_cycles, sheet_name=None):
    try:
        gc = _make_gspread_client_from_b64()
        if not gc:
            logging.warning('gspread client unavailable; storing mission_cycles in-memory only.')
            global _LOADED_MISSION_CYCLES
            _LOADED_MISSION_CYCLES = mission_cycles.copy()
            return False
        sheet_name = sheet_name or os.environ.get('GOOGLE_SHEET_NAME') or os.environ.get('SHEET_ID') or 'Bot_State'
        sh = gc.open(sheet_name)
        tab = os.environ.get('BOT_STATE_TAB','Bot_State')
        try:
            ws = sh.worksheet(tab)
        except Exception:
        ...
        rows = [['Key','Value']]
        for k,v in mission_cycles.items():
            rows.append([k, json.dumps(v)])
        ws.clear()
        ws.update('A1', rows)
        global _LOADED_MISSION_CYCLES
        _LOADED_MISSION_CYCLES = mission_cycles.copy()
        logging.info('Saved mission cycles to sheet.')
        return True
    except Exception:
        logging.exception('Failed to save mission cycles to sheet; saved in memory.')
        _LOADED_MISSION_CYCLES = mission_cycles.copy()
        return False
...
def load_mission_cycles_from_sheet(sheet_name=None):
    try:
        gc = _make_gspread_client_from_b64()
        if not gc:
            logging.warning('gspread client unavailable; loading in-memory mission cycles.')
        ...
        global _LOADED_MISSION_CYCLES
        _LOADED_MISSION_CYCLES = result.copy()
        logging.info('Loaded mission cycles from sheet.')
        return result
    except Exception:
        logging.exception('Failed to load mission cycles from sheet; using in-memory.')
        return _LOADED_MISSION_CYCLES.copy()

def increment_mission_cycle_pairing(driver, plate, departure, arrival):
    # Pair PP->SHV then SHV->PP as a roundtrip. Update mission cycles and persist.
    global _LOADED_MISSION_CYCLES
    mission_cycles = _LOADED_MISSION_CYCLES.copy()
    key = f"mission_cycle|{driver}|{plate}"
    state_key = f"mission_state|{driver}|{plate}"
    last_state = mission_cycles.get(state_key)
    if departure == 'PP' and arrival == 'SHV':
        mission_cycles[state_key] = 'outbound'
        mission_cycles[key] = mission_cycles.get(key, 0) + 1
        logging.debug(f'Outbound recorded for {driver} {plate}. count={mission_cycles[key]}')
    elif departure == 'SHV' and arrival == 'PP':
        if last_state == 'outbound':
            mission_cycles[state_key] = 'completed'
            mission_cycles[key] = mission_cycles.get(key, 0) + 1
            logging.info(f'Roundtrip completed for {driver} {plate}. count={mission_cycles[key]}')
            try:
                if 'on_roundtrip_complete' in globals() and callable(globals()['on_roundtrip_complete']):
                    globals()['on_roundtrip_complete'](driver, plate, mission_cycles[key], mission_cycles)
            except Exception:
                logging.exception('on_roundtrip_complete hook failure')
        else:
            mission_cycles[state_key] = 'inbound'
            mission_cycles[key] = mission_cycles.get(key, 0) + 1
            logging.debug(f'Inbound recorded for {driver} {plate}. count={mission_cycles[key]}')
    else:
        mission_cycles[state_key] = f"{departure}->{arrival}"
        mission_cycles[key] = mission_cycles.get(key, 0)
    _LOADED_MISSION_CYCLES = mission_cycles
    try:
        save_mission_cycles_to_sheet(mission_cycles)
    except Exception:
        logging.exception('Failed to persist mission cycles after increment.')
    return mission_cycles.get(key,0)
