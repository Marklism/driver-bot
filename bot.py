...
# import helpers generated by assistant
from bot_helpers_fixed import (
    load_mission_cycles_from_sheet,
    save_mission_cycles_to_sheet,
    increment_mission_cycle_pairing,
    record_clock_entry,
    process_clock_out_and_calc_ot,
    on_roundtrip_complete,
)

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger('driver-bot-merged')

BOT_TOKEN = os.getenv('BOT_TOKEN')
if not BOT_TOKEN:
    logger.warning('BOT_TOKEN not set; the bot cannot talk to Telegram.')
else:
    logger.info('BOT_TOKEN present.')

# Optional: override on_roundtrip_complete to send a Telegram message
async def _notify_roundtrip_complete(driver: str, plate: str, count: int, mission_cycles):
    # Try to send via bot if token provided. Replace CHAT_ID with your group/chat id or pass in command.
    chat_id = os.getenv('SUMMARY_CHAT_ID')  # set to your chat id or leave empty to only log
    text = f"Roundtrip complete: driver={driver} plate={plate} count={count}"
    logger.info('Roundtrip: %s', text)
    if BOT_TOKEN and chat_id:
        try:
            bot = Bot(BOT_TOKEN)
            await bot.send_message(chat_id=int(chat_id), text=text)
        except Exception:
            logger.exception('Failed to send roundtrip notification via Telegram.')
    else:
        logger.info('No SUMMARY_CHAT_ID or BOT_TOKEN: roundtrip notification logged only.')

# Replace helper hook with a wrapper that schedules the async send above
def _hook_on_roundtrip_complete(driver, plate, count, mission_cycles):
    try:
        # call asynchronously via Application if available; fallback to logging
        import asyncio
        try:
            asyncio.create_task(_notify_roundtrip_complete(driver, plate, count, mission_cycles))
        except Exception:
            # if no running loop, run sync
            import nest_asyncio
            try:
                nest_asyncio.apply()
            except Exception:
            ...
            loop.run_until_complete(_notify_roundtrip_complete(driver, plate, count, mission_cycles))
    except Exception:
        logger.exception('Failed in hook scheduling for roundtrip notification.')

# Patch the helper's hook
try:
    # set global in helper module
    import bot_helpers_fixed as helpers_mod
    helpers_mod.on_roundtrip_complete = _hook_on_roundtrip_complete
except Exception:
    logger.exception('Failed to patch on_roundtrip_complete hook in helpers module.')
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text('Driver-bot merged running. Commands: /mission_start, /mission_end, /clock_in, /clock_out')

async def mission_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Usage: /mission_start driver plate dep
    try:
        args = context.args
        if len(args) < 3:
            await update.message.reply_text('Usage: /mission_start <driver> <plate> <dep>')
            return
        driver, plate, dep = args[0], args[1], args[2]
        # Record in mission cycle as outbound if PP->SHV etc.
        count = increment_mission_cycle_pairing(driver, plate, dep, dep)  # pass same for arrival to just set state
        await update.message.reply_text(f'Recorded mission start for {driver} {plate} dep={dep}.')
    except Exception:
        logger.exception('mission_start handler failed.')
        await update.message.reply_text('Error recording mission start.')

async def mission_end(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Usage: /mission_end driver plate dep arr
    try:
        args = context.args
        if len(args) < 4:
            await update.message.reply_text('Usage: /mission_end <driver> <plate> <dep> <arr>')
            return
        driver, plate, dep, arr = args[0], args[1], args[2], args[3]
        count = increment_mission_cycle_pairing(driver, plate, dep, arr)
        # respond with current count
        await update.message.reply_text(f'Mission end recorded for {driver} {plate} {dep}->{arr}. roundtrip_count={count}')
    except Exception:
        logger.exception('mission_end handler failed.')
        await update.message.reply_text('Error recording mission end.')

async def clock_in(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Usage: /clock_in driver
    try:
        args = context.args
        if len(args) < 1:
            await update.message.reply_text('Usage: /clock_in <driver>')
            return
        driver = args[0]
        row = record_clock_entry(driver, 'IN', note='telegram_clock_in')
        await update.message.reply_text(f'Clock IN recorded for {driver}: {row}')
    except Exception:
        logger.exception('clock_in handler failed.')
        await update.message.reply_text('Error recording clock in.')

async def clock_out(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Usage: /clock_out driver
    try:
        args = context.args
        if len(args) < 1:
            await update.message.reply_text('Usage: /clock_out <driver>')
            return
        driver = args[0]
        row = record_clock_entry(driver, 'OUT', note='telegram_clock_out')
        result = process_clock_out_and_calc_ot(driver)
        if result:
            await update.message.reply_text(f'Clock OUT recorded for {driver}. OT hours={result.get("ot_hours")}')
            # optional: notify summary chat
            chat_id = os.getenv('SUMMARY_CHAT_ID')
            if BOT_TOKEN and chat_id:
                bot = Bot(BOT_TOKEN)
                await bot.send_message(chat_id=int(chat_id), text=f'Driver {driver} OT recorded: {result.get("ot_hours")} hours.')
        else:
            await update.message.reply_text('Clock OUT recorded but no OT calculated (no prior IN found or sheet unavailable).')
    except Exception:
        logger.exception('clock_out handler failed.')
        await update.message.reply_text('Error recording clock out.')

def build_app():
    token = BOT_TOKEN
    if not token:
        raise RuntimeError('BOT_TOKEN undefined. Set env var BOT_TOKEN before running.')
    app = ApplicationBuilder().token(token).build()
    app.add_handler(CommandHandler('start', start))
    app.add_handler(CommandHandler('mission_start', mission_start))
...
# --- APPENDED FROM bot_fixed_for_user.py (automated merge) ---
# New globals/imports copied from extra file preamble
...
# Appending 75 missing definitions: check_deployment_requirements, record_clock_entry, get_last_clock_entry, _is_weekend, compute_ot_for_shift, clock_callback_handler, ot_report_command, GoogleApiQueue, WorksheetProxy, t, _load_creds_from_base64, get_gspread_client, ensure_sheet_has_headers_conservative, ensure_sheet_headers_match, _missions_header_fix_if_needed, open_worksheet, process_leave_entry, load_driver_map_from_env, load_driver_map_from_sheet, get_driver_map, _now_dt, now_str, today_date_str, parse_ts, compute_duration, record_start_trip, record_end_trip, _missions_get_values_and_data_rows, _missions_next_no, _ensure_row_length, start_mission_record, end_mission_record, mission_rows_for_period, write_mission_report_rows, count_roundtrips_per_driver_month, count_trips_for_day, count_trips_for_month, normalize_fin_type, _find_last_mileage_for_plate, record_finance_odo_fuel, record_parking, record_wash, record_repair, build_plate_keyboard, safe_delete_message, menu_command, start_trip_command, end_trip_command, mission_start_command, mission_end_command, leave_command, admin_finance_callback_handler, admin_fin_type_selected, process_force_reply, location_or_staff, plate_callback, lang_command, mission_report_command, debug_bot_command, auto_menu_listener, send_daily_summary_job, aggregate_for_period, setup_menu_command, delete_command_message, handle_clock_button, register_ui_handlers, ensure_env, schedule_daily_summary, _delete_telegram_webhook, _send_startup_debug, _make_gspread_client_from_b64, save_mission_cycles_to_sheet, load_mission_cycles_from_sheet, increment_mission_cycle_pairing, on_roundtrip_complete
