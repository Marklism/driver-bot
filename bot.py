# --- BEGIN: gspread API queue + aggressive caching monkeypatch ---
import threading, time, collections, functools, traceback, sys

try:
    # gspread Worksheet class (may differ by version)
    import gspread
    from gspread import worksheet as _g_worksheet
    WorksheetClass = getattr(_g_worksheet, "Worksheet", None)
except Exception:
    WorksheetClass = None

# Queue to serialize all gspread API calls; each call is executed by the worker thread.
_GS_API_QUEUE = collections.deque()
_GS_QUEUE_LOCK = threading.Lock()
_GS_WORKER_THREAD_STARTED = False
# Minimum delay between calls (seconds) - keep safely under Google per-minute quota
_GS_MIN_DELAY = float(int(os.getenv("GS_MIN_DELAY_MS", "1100")))/1000.0

# Aggressive read cache: caches get_all_values / get_all_records results keyed by worksheet id or title.
_GS_READ_CACHE = {}
_GS_READ_TTL = float(int(os.getenv("GS_READ_TTL_SEC", "5")))  # seconds

# Append buffer per worksheet (write cache). Flush interval seconds.
_GS_APPEND_BUFFER = {}
_GS_APPEND_LOCK = threading.Lock()
_GS_FLUSH_INTERVAL = float(int(os.getenv("GS_APPEND_FLUSH_SEC", "2")))
_GS_MAX_BATCH = int(os.getenv("GS_APPEND_MAX_BATCH", "25"))

# Save original methods for fallback
_orig_methods = {}

def _start_worker():
    global _GS_WORKER_THREAD_STARTED
    if _GS_WORKER_THREAD_STARTED or WorksheetClass is None:
        return
    _GS_WORKER_THREAD_STARTED = True
    ...
    def worker():
        ...
        while True:
            ...
            with _GS_QUEUE_LOCK:
                if _GS_API_QUEUE:
                    item = _GS_API_QUEUE.popleft()
            if not item:
                time.sleep(0.2)
                continue
            func, args, kwargs, ev = item
            # ensure spacing
            wait = _GS_MIN_DELAY - (time.time() - last_ts)
            if wait > 0:
                time.sleep(wait)
            try:
                res = func(*args, **kwargs)
                item_res = (True, res, None)
            except Exception as e:
                # return exception info
                item_res = (False, None, e)
            last_ts = time.time()
            ev["res"] = item_res
            ev["event"].set()

    t = threading.Thread(target=worker, daemon=True, name="gsheets-queue-worker")
    t.start()

    # start flusher thread for append buffer
    def flusher():
        while True:
            time.sleep(_GS_FLUSH_INTERVAL)
            try:
                keys = []
                with _GS_APPEND_LOCK:
                    for wid, buf in list(_GS_APPEND_BUFFER.items()):
                        if not buf:
                            continue
                        keys.append(wid)
                        rows = buf[:_GS_MAX_BATCH]
                        del _GS_APPEND_BUFFER[wid][:len(rows)]
                        # schedule actual append calls in queue
                        for row in rows:
                            ws_obj = row["_ws_obj"]
                            _enqueue_api_call(getattr(ws_obj, "_orig_append_row"), (row["row"],), {"value_input_option": row.get("value_input_option", "USER_ENTERED")})
            except Exception:
                traceback.print_exc(file=sys.stderr)

    tf = threading.Thread(target=flusher, daemon=True, name="gsheets-append-flusher")
    tf.start()

def _enqueue_api_call(func, args=(), kwargs=None):
    if kwargs is None:
        kwargs = {}
    ev = {"event": threading.Event(), "res": None}
    with _GS_QUEUE_LOCK:
        _GS_API_QUEUE.append((func, args, kwargs, ev))
    # ensure worker started
    _start_worker()
    # wait for completion and return or raise
    ev["event"].wait()
    ok, res, exc = ev["res"]
    if ok:
        return res
...
def _cache_get(ws_key):
    ent = _GS_READ_CACHE.get(ws_key)
    if not ent:
        return None
    ts, data = ent
    if time.time() - ts < _GS_READ_TTL:
        return data
    # expired
    del _GS_READ_CACHE[ws_key]
    return None

def _cache_set(ws_key, data):
    _GS_READ_CACHE[ws_key] = (time.time(), data)

# wrapper for get_all_values
def _wrap_get_all_values(self, *args, **kwargs):
    try:
        ws_key = getattr(self, "id", None) or getattr(self, "title", None) or str(self)
        cached = _cache_get(ws_key)
        if cached is not None:
            return cached
        # call underlying original method through queue to respect rate limits
        func = getattr(self, "_orig_get_all_values") if hasattr(self, "_orig_get_all_values") else getattr(self, "get_all_values", None)
        if not func:
            return []
        res = _enqueue_api_call(func, args, kwargs)
        try:
            _cache_set(ws_key, res)
        except Exception:
            pass
        return res
    except Exception:
        ...
        try:
            return self._orig_get_all_values(*args, **kwargs)
        except Exception:
            return []

def _wrap_get_all_records(self, *args, **kwargs):
    try:
        ws_key = getattr(self, "id", None) or getattr(self, "title", None) or str(self)
        cached = _cache_get(ws_key + "|records")
        if cached is not None:
            return cached
        func = getattr(self, "_orig_get_all_records") if hasattr(self, "_orig_get_all_records") else getattr(self, "get_all_records", None)
        if not func:
            return []
        res = _enqueue_api_call(func, args, kwargs)
        try:
            _cache_set(ws_key + "|records", res)
        except Exception:
            pass
        return res
    except Exception:
        try:
            return self._orig_get_all_records(*args, **kwargs)
        except Exception:
            return []

def _wrap_row_values(self, *args, **kwargs):
    # small cache by worksheet id + row index
    try:
        key = getattr(self, "id", None) or getattr(self, "title", None) or str(self)
        idx = None
        if args:
            idx = args[0]
        cache_key = f"{key}|row|{idx}"
        cached = _cache_get(cache_key)
        if cached is not None:
            return cached
        func = getattr(self, "_orig_row_values") if hasattr(self, "_orig_row_values") else getattr(self, "row_values", None)
        if not func:
            return []
        res = _enqueue_api_call(func, args, kwargs)
        try:
        ...
        try:
            return self._orig_row_values(*args, **kwargs)
        except Exception:
            return []

def _wrap_update_cell(self, *args, **kwargs):
    # Clear relevant caches on write
    try:
        key = getattr(self, "id", None) or getattr(self, "title", None) or str(self)
        # perform update via queue
        func = getattr(self, "_orig_update_cell") if hasattr(self, "_orig_update_cell") else getattr(self, "update_cell", None)
        if not func:
            return None
        res = _enqueue_api_call(func, args, kwargs)
        # invalidate caches for this worksheet
        try:
            _GS_READ_CACHE.pop(key, None)
        except Exception:
            pass
        return res
    except Exception:
        try:
            return self._orig_update_cell(*args, **kwargs)
        except Exception:
            return None

def _wrap_insert_row(self, *args, **kwargs):
    try:
        func = getattr(self, "_orig_insert_row") if hasattr(self, "_orig_insert_row") else getattr(self, "insert_row", None)
        if not func:
            return None
        res = _enqueue_api_call(func, args, kwargs)
        # invalidate cache
        ...
        try:
            return self._orig_insert_row(*args, **kwargs)
        except Exception:
            return None

def _wrap_delete_rows(self, *args, **kwargs):
    try:
        func = getattr(self, "_orig_delete_rows") if hasattr(self, "_orig_delete_rows") else getattr(self, "delete_rows", None)
        if not func:
            return None
        res = _enqueue_api_call(func, args, kwargs)
        key = getattr(self, "id", None) or getattr(self, "title", None) or str(self)
        ...
        try:
            return self._orig_delete_rows(*args, **kwargs)
        except Exception:
            return None

def _wrap_append_row(self, row, *args, **kwargs):
    # Use append buffer: add to buffer and return immediately (simulating success).
    try:
        key = getattr(self, "id", None) or getattr(self, "title", None) or str(self)
        with _GS_APPEND_LOCK:
            if key not in _GS_APPEND_BUFFER:
                _GS_APPEND_BUFFER[key] = []
            _GS_APPEND_BUFFER[key].append({"row": row, "value_input_option": kwargs.get("value_input_option", "USER_ENTERED"), "_ws_obj": self})
            # if buffer length exceeds threshold, flush synchronously by scheduling queued append calls
            if len(_GS_APPEND_BUFFER[key]) >= _GS_MAX_BATCH:
                rows = _GS_APPEND_BUFFER[key][:_GS_MAX_BATCH]
                del _GS_APPEND_BUFFER[key][:len(rows)]
                for r in rows:
                    _enqueue_api_call(getattr(self, "_orig_append_row"), (r["row"],), {"value_input_option": r.get("value_input_option", "USER_ENTERED")})
        # invalidate read cache for this sheet (so subsequent reads will reflect new data eventually)
        try:
            _GS_READ_CACHE.pop(key, None)
        except Exception:
            pass
        # return a best-effort ack (gspread append_row typically returns None)
        return None
    except Exception:
        # fallback to queued direct append
        try:
            return _enqueue_api_call(getattr(self, "_orig_append_row"), (row,), kwargs)
        except Exception:
            return None

# Monkeypatch the Worksheet class to route calls through the queue/cache
def _apply_monkeypatch():
    if WorksheetClass is None:
        return
    mapping = [
        ("get_all_values", _wrap_get_all_values),
        ("get_all_records", _wrap_get_all_records),
        ("row_values", _wrap_row_values),
        ("update_cell", _wrap_update_cell),
        ("insert_row", _wrap_insert_row),
        ("delete_rows", _wrap_delete_rows),
        ("append_row", _wrap_append_row),
    ]
    for name, wrapper in mapping:
        if hasattr(WorksheetClass, name):
            orig = getattr(WorksheetClass, name)
            setattr(WorksheetClass, "_orig_" + name, orig)
            setattr(WorksheetClass, name, wrapper)
    # also ensure get is routed through get_all_values where appropriate by keeping original get intact.
    _start_worker()

_apply_monkeypatch()
# --- END: gspread API queue + aggressive caching monkeypatch ---
